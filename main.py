...
for k in global_weights.keys():
	...
	D1[k] = torch.zeros(K,K, dtype=global_weights[k].dtype)
...
for k in global_weights.keys():
	for i in range(len(final_updates)):
		...
		#print(mean[k])
		#if i==len(final_updates)-K:
		#D[k]=final_updates[i][k]-mean[k]
		std[k] =(i*std[k]+torch.mul(final_updates[i][k], final_updates[i][k]))/(i+1)
		if len(final_updates)-i<K:
			#d[k]+=torch.mul(final_updates[i][k]-mean[k])
			D1[k]+=torch.ger(final_updates[i][k]-mean[k],final_updates[i][k]-mean[k])
			#D=torch.cat(D,final_updates[i][k]-mean[k],dim=0)
...
for j in range(num_samples):
	...
	for k in swag_model.keys():
		swag_model[k] = mean[k]+(0.5**0.5)*std[k]*torch.normal(torch.zeros(global_weights[k].shape,dtype=global_weights[k].dtype),torch.eye(global_weights[k].shape))+((2*(K-1))**0.5)*D1[k]*torch.normal(torch.zeros(D1[k][0].shape,dtype=D1[k][0].dtype),torch.eye(D1[k].shape))
		#print('swag_model sampled',swag_model[k])
		#print('swag_model mean',mean[k])
		#print('swag_model std',std[k])
